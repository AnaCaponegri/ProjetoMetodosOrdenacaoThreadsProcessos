    ...
    public synchronized List<ArquivoInfo> listarArquivos() { 
        ...
        if (arquivos != null) { 
            for (File arquivo : arquivos) { 
                if (arquivo.isFile()) { 
                    int downloads = 
contadorDownloads.getOrDefault(arquivo.getName(), 0); 
 
                    lista.add(new ArquivoInfo( 
                            arquivo.getName(), 
                            arquivo.length(), 
                            downloads 
                    )); 
    ...
    public synchronized void incrementarDownload(String nomeArquivo) { 
        contadorDownloads.put( 
                nomeArquivo, 
                contadorDownloads.getOrDefault(nomeArquivo, 0) + 1 
        ); 
    } 
    ...
    public String gerarListaTexto() { 
        ...
        for (ArquivoInfo arquivo : listarArquivos()) { 
            sb.append(arquivo.getNome()) 
              .append(";") 
              .append(arquivo.getTamanho()) 
              .append(";") 
              .append(arquivo.getDownloads()) 
              .append("\n"); 
        } 
